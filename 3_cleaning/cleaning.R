...
country <- read.csv("C:/Users/21260/Desktop/Dataintegration/project/last_ds/Country.csv")
unemployement <- read.csv("C:/Users/21260/Desktop/Dataintegration/project/last_ds/Unemployement.csv")
happiness <- read.csv("C:/Users/21260/Desktop/Dataintegration/project/last_ds/Happiness.csv")
poverty <- read.csv("C:/Users/21260/Desktop/Dataintegration/project/last_ds/Poverty.csv")

#### na detection 
any(is.na(country))
any(is.na(unemployement))
any(is.na(happiness))
any(is.na(poverty))


## detecting the nas
which(is.na(country), arr.ind=TRUE) #nas
which(is.na(unemployement), arr.ind=TRUE) #no nas
which(is.na(happiness), arr.ind=TRUE) #nas
which(is.na(poverty), arr.ind=TRUE) #no nas


### which entries are not relevant 
not_relevant_countries <- filter(country,is.na(country$Country_code))
not_relevant_happiness <- filter(happiness,is.na(happiness$Country_code))
##writing 
write.csv(not_relevant_countries, "C:/Users/21260/Desktop/Dataintegration/project/last_ds/nrc.csv", row.names=FALSE)
write.csv(not_relevant_happiness, "C:/Users/21260/Desktop/Dataintegration/project/last_ds/nrh.csv", row.names=FALSE)


## deal with nas in country and happiness 
## remove them because they do not represent the country entity (areas)
country_nar <- na.omit(country)
happiness_nar <- na.omit(happiness)


## checking nrows lengths for intersection detection
nrow(country_nar)
length(unique(happiness$Country))
length(unique(unemployement$Country))
length(unique(poverty$Country))

## intersection bilden
data_provided <- intersect(intersect(happiness$Country,unemployement$Country),poverty$Country)

## clean non matching data
library(dplyr)
country_cleaned <- country_nar[country_nar$Country %in% toupper(data_provided),]
unemployement_cleaned <- filter(unemployement,unemployement$Country %in% data_provided)
happiness_cleaned <- filter(happiness_nar,happiness_nar$Country %in% data_provided)
poverty_cleaned <- filter(poverty,poverty$Country %in% data_provided)


merge1 <- merge(happiness_cleaned,poverty_cleaned,by = intersect(names(happiness_cleaned),names(poverty_cleaned)))
final_ds <- merge(unemployement_cleaned,merge1,by = intersect(names(merge1),names(unemployement_cleaned)))

## lets check for duplicates/intersections in country
data_2015 <- filter(final_ds,year == 2015)
data_2016 <- filter(final_ds,year == 2016)
data_2017 <- filter(final_ds,year == 2017)
data_2018 <- filter(final_ds,year == 2018)
data_2019 <- filter(final_ds,year == 2019)


## in the end we have those countries
countries <- Reduce(intersect,list(data_2015$Country,data_2016$Country,data_2017$Country,data_2018$Country,data_2019$Country))

final_cleaned_ds <- filter(final_ds,Country %in% countries)

final_cleaned_ds$Corruption <- as.numeric(data_2015$Corruption)

### data for each year 
data_2015 <- filter(final_cleaned_ds,year == 2015)
data_2016 <- filter(final_cleaned_ds,year == 2016)
data_2017 <- filter(final_cleaned_ds,year == 2017)
data_2018 <- filter(final_cleaned_ds,year == 2018)
data_2019 <- filter(final_cleaned_ds,year == 2019)
...
classes <- col_classes(data_2015)
classes ## need to convert corruption


### writing into csv 
write.csv(final_cleaned_ds,"final_data.csv",row.names = F)
